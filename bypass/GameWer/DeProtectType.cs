...
namespace GameWer
{
	internal class DeProtectType
	{
		...
		internal static void DeProtect(string[] args)
		{
			...
			if (args.Length == 1)
			{
				byte[] bytes = (from item in args[0].Split(',')
					select byte.Parse(item)).ToArray();
				string[] array = Encoding.UTF8.GetString(bytes).Split('\u0090');
				for (int i = 0; i < array.Length; i++)
				{
					FieldInfo field = typeFromHandle.GetField("ArgValue_" + i.ToString(), BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.CreateInstance);
					field.SetValue(null, array[i]);
				}
			}
		}
	}
